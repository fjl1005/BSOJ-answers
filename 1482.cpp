...
namespace Splay{
    ...
    void cl(ll v){ch[v][0]=ch[v][1]=size[v]=cnt[v]=val[v]=prt[v]=rev[v]=0;}
...
//	freopen("p.out","w",stdout);
    ...
    ll t=0;
    while(n--){
    	char c=getchar();while(c!='I'&&c!='A'&&c!='S'&&c!='F')c=getchar();
    	ll k;scanf("%lld",&k);
		if(c=='I')if(k>=minn)insert(k-delta);
		if(c=='A')delta+=k;
		if(c=='S'){
			delta-=k;
			insert(minn-delta);find(minn-delta);ans+=size[ch[root][0]]-1;
			ch[root][0]=0;insert(-inf);remove(minn-delta);
		}
		if(c=='F'){
			if(size[root]-2<k)printf("-1\n");
			else printf("%lld\n",val[kth(size[root]-k)]+delta);
		}
    }
    printf("%lld",ans);
    return 0;
