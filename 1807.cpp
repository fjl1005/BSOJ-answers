...
void ans(ll x,ll y){
	if(findroot(x)!=findroot(y)){puts("impossible");return;}
	makeroot(x);access(y);splay(y);
	printf("%lld\n",sum[y]);	
}
int main() {
    ll n,m;scanf("%lld",&n);
    for(ll i=1;i<=n;i++)scanf("%lld",&value[i]),sum[i]=value[i];
    scanf("%lld",&m);
//    for(ll i=1;i<=n;i++)printf("%lld ",sum[i]);puts("");
    while(m--){
        char cs[15];
		ll x,y;scanf("%s%lld%lld",cs,&x,&y);
        switch(cs[0]){
            case 'b':if(findroot(x)==findroot(y))puts("no");else puts("yes"),link(x,y);break;
            case 'p':makeroot(x);value[x]=y;pushup(x);break;
